...
| **O registro tem teto, e o caderno de quem joga esquecia junto** | ✅ `dossie_log` guarda 60 linhas, e o bloco assistido deriva do registro DO ZERO a cada renderização. Uma partida deste banco chegou a `seq` **281**: 221 linhas caíram, e com elas todo *"fulano não tem nenhuma das três"* que provavam. Medido: **54 marcas com o registro inteiro, 9 com o cortado.** A máquina não perdia nenhuma, porque `dossie_deduz` é incremental — a assimetria que o PRD diz não querer, e não por o bloco ser fraco: por ele ser AMNÉSICO |
| E o teste foi escrito antes do conserto — e a primeira versão mentiu | ✅ os palpites ciclavam por resto de divisão, e as doze rodadas finais repetiam todas as cartas das oito primeiras: o corte não custava nada e o teste dizia que estava tudo bem. Ele media a própria fixture. Numa partida de verdade a mesa vasculha no começo e insiste no fim |
| A suíte parou de morrer de DNS | ✅ `pg.Pool` resolveu a conexão que MORRE no meio; a que nunca chega a existir subia direto pelo `db.query` e matou sete minutos de suíte do Dossiê ao lado de trezentas linhas verdes |
| E as outras duas sequências cronometradas | ✅ a **revelação do Letreiro** (2,2s por palavra que escapou — numa grade rica, mais de quarenta segundos, e o botão "Pular" ia direto para o placar: quem pediu menos movimento escolhia entre esperar ou PERDER a lista) e a **abertura do Dossiê** (quase trinta segundos, e a narração é a única parte do caso que não se recupera depois). Nas três, `calmo` entra como prop, então as seis montagens passam pelas cinco auditorias do HTML |
| **Tempo não é CSS** | ✅ o desligamento universal de `prefers-reduced-motion` tirava o giro do dado do Domínio e não tirava um milissegundo da encenação: 620ms + 1150ms por assalto, doze assaltos, **vinte e um segundos de dados parados** para quem pediu menos movimento — pior que a animação que ela não queria. Agora o painel mostra os assaltos todos de uma vez e quem fecha é a pessoa |
| E `npm run css` guarda o desligamento | ✅ 48 regras animam, e a promessa do PRD 01 é cumprida por UM bloco. Apagá-lo devolve as 48 a quem pediu que não houvesse nenhuma, e nada acusaria — a tela continua perfeita para quem não ligou a preferência. A varredura ingênua (um bloco de `reduce` por regra animada) dá doze falsos positivos, e eu escrevi a ingênua primeiro |
| **Metade de cada tela de jogo nunca tinha sido desenhada** | ✅ reforçar, atacar, comprar, construir, encerrar o turno — todo esse ramo só existe QUANDO É A SUA VEZ, e a montagem não tinha sessão. Nenhuma das cinco auditorias do HTML tinha visto um daqueles botões. `SessionContext` passou a ser exportado para um consumidor só, e a suíte serve a sessão do assento da vez |
| E ela media a sorte | ✅ a partida achada no banco está na fase em que a última suíte a deixou, então o que a montagem cobria dependia da linha que o banco tinha naquela hora — foi assim que a auditoria de contraste veio achando um defeito por rodada, com verde no meio. Agora são nove telas inteiras no lugar de três, as mesmas em toda rodada, e **1.335 pedaços de texto medidos** contra 1.033 |
| **O anfitrião fechava o navegador e a mesa travava** | ✅ migração 0120. A passagem de anfitrião só acontecia dentro de `leave_room` — quando a pessoa APERTA sair. Fechar a aba deixava a sala com um anfitrião que não existe, e só ele começa a partida. **Duas trancas:** o cliente pede quando a presença do Realtime avisa, o servidor concede quando o pulso vence. O pulso do lobby caiu de 30s para 10s por causa disso |
| Fechar a aba no meio da partida, nos três jogos | ✅ e o que a pessoa digita depressa é sempre o candidato a virar estado de React: a lista do Letreiro, o bloco do Dossiê, o objetivo secreto do Domínio. Funciona até o ônibus entrar no túnel |
| O crivo de temas do PRD 07 §5 passou a rodar no CI | ✅ ele só olhava os pacotes na hora de PUBLICÁ-LOS, e achou um buraco na primeira leitura do que está publicado |
