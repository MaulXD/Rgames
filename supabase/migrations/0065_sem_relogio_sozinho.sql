-- ════════════════════════════════════════════════════════════════════════════
-- Mesa — 0065 · o relógio não corre contra quem está sozinho
--
-- O relógio do turno existe por um motivo só: proteger AS OUTRAS PESSOAS de
-- quem sumiu. Numa mesa em que todo o resto é máquina, não há quem proteger — e
-- o que o relógio faz ali é tirar o turno de quem atendeu o telefone.
--
-- E no celular esse é o caso COMUM, não o raro: sair do aplicativo já é sair da
-- aba, e o modo solo é onde isso mais acontece. Uma pessoa jogando Domínio
-- sozinho no ônibus perderia um turno a cada dois minutos de conversa.
--
-- Então, quando quem está na vez é gente e não sobrou mais nenhuma pessoa na
-- mesa, o relógio é DESLIGADO — `turn_deadline` vai a nulo — em vez de correr.
-- A máquina continua tendo relógio: ela nunca some, mas se o cérebro dela
-- falhar, a rede de 0063 ainda passa a vez.
--
-- No Dossiê a guarda vem DEPOIS da fase de refutação, de propósito: se alguém
-- precisa refutar, a fila tem de andar mesmo numa mesa solo — senão a partida
-- para esperando uma máquina que já respondeu.
-- ════════════════════════════════════════════════════════════════════════════

/**
 * Nesta partida, sobrou mais alguma PESSOA além do assento dado?
 *
 * Falso quando todo o resto é máquina — e é aí que o relógio deixa de fazer
 * sentido. Fantasma e quebrado continuam contando como pessoa: eles ainda estão
 * na mesa, ainda refutam, ainda leiloam.
 */
create or replace function public.mesa_so_com_maquinas(p_match uuid, p_seat smallint)
returns boolean
language sql
stable
security definer
  ...
  select not exists (
    select 1
     ...
     where mp.match_id = p_match
       and mp.seat is distinct from p_seat
       and not p.is_bot
  )
  and exists (
    select 1
      from public.match_players mp
      join public.profiles p on p.id = mp.user_id
     where mp.match_id = p_match and mp.seat = p_seat and not p.is_bot
  );
$$;

revoke all on function public.mesa_so_com_maquinas(uuid, smallint)
