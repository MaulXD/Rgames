-- ════════════════════════════════════════════════════════════════════════════
-- Mesa — 0079 · a quarta vez, e por isso agora tem auditoria
--
-- `botProp` — o teto de uma proposta por rodada que 0070 criou para matar o laço
-- de propor-recusar — nunca era gravado. Mesmo `jsonb_set` com pai ausente de
-- 0044 e 0077: o caminho `{botProp, assento}` tem um degrau faltando no meio, e
-- `create_missing` cria só o último.
--
-- Então o teto nunca valia, e o laço voltou inteiro:
--
--     propoe: 1170     troca:recusa: 1170     em 2400 passos
--
-- QUARTA OCORRÊNCIA DO MESMO DEFEITO nesta sessão: `botTempos` (0044),
-- `tregProp` e `treguas` (0077), `multaReforco` (0078) e agora `botProp`. Cada
-- uma delas tem um comentário explicando a armadilha, escrito por mim, algumas
-- linhas acima de onde eu caí nela de novo.
--
-- A conclusão é a mesma do `seat` em 0069: comentário não é mecanismo. O que
-- vale é `jsonb_poe`, que faz a coisa certa e é mais curta de escrever do que a
-- errada — e a auditoria em scripts/smoke.mjs, que lista todas as chaves usadas
-- em caminho de dois níveis e reprova quando aparece uma que ninguém garantiu
-- que existe.
-- ════════════════════════════════════════════════════════════════════════════
    ...
    if prop is not null then
      ...
      if oferta >= coalesce((
        ...
        begin
          ...
          /* `jsonb_poe` e não `jsonb_set`: `botProp` não existe no estado inicial,
             e `create_missing` cria só o último degrau do caminho. A gravação
             voltava intacta, o teto de uma proposta por rodada nunca valia, e o
             laço de propor-recusar de 0070 voltava inteiro — medido em 1170
             propostas em 2400 passos. Ver 0077. */
          update public.matches
             set public_state = public.jsonb_poe(
                   public_state, 'botProp', assento::text, to_jsonb(rodada))
           where id = p_match;
