-- ════════════════════════════════════════════════════════════════════════════
-- Mesa — 0082 · a máquina responde trégua FORA da vez dela
--
-- 0081 pôs a resposta de trégua dentro de `dominio_bot_passo` — e depois da
-- linha que diz "se não é vez de máquina, volta sem fazer nada".
--
-- É onde ela nunca roda. Uma proposta de trégua chega no turno de QUEM PROPÔS, e
-- quem propõe quase sempre é a pessoa: então no instante em que a proposta
-- existe, é vez de gente, e a função já voltou.
--
-- Medido: proposta enviada, `dominio_bot_passo` devolveu `null`, e a proposta
-- ficou pendurada no estado.
--
-- O bloco sobe para antes da checagem, ao lado de onde a Metrópole trata leilão
-- e proposta de troca — e pelo mesmo motivo. As três coisas têm a mesma forma:
-- são AÇÕES QUE NÃO SÃO DO TURNO, e a checagem de turno tem de vir depois delas.
--
-- Vale como regra: num cérebro de máquina, tudo que a mesa pode exigir fora da
-- vez vem antes de "é a minha vez?".
-- ════════════════════════════════════════════════════════════════════════════
