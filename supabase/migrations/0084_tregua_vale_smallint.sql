-- ════════════════════════════════════════════════════════════════════════════
-- Mesa — 0084 · `coalesce(smallint, -1)` é integer
--
--     function public.dominio_tregua_vale(jsonb, smallint, integer) does not exist
--
-- `coalesce((x)::smallint, -1)` não devolve smallint: o literal `-1` é integer, e
-- o coalesce promove o resultado inteiro para o tipo mais largo. A função é
-- `(jsonb, smallint, smallint)`, e o Postgres não encontra a assinatura.
--
-- O mesmo padrão `coalesce(..., -1)` aparece dezenas de vezes neste projeto e
-- nunca deu problema, porque em todas elas o resultado ia para uma COMPARAÇÃO
-- (`<> assento`), onde a promoção é invisível. Aqui ele virou ARGUMENTO — e
-- argumento tem tipo.
-- ════════════════════════════════════════════════════════════════════════════
    ...
    select v.ter, x.viz, v.meus - 1, x.nviz
     ...
     where v.meus >= 2
       ...
       and not public.dominio_tregua_vale(
             est, assento, coalesce((est -> 'donos' ->> x.viz)::smallint, -1)::smallint)
     order by
