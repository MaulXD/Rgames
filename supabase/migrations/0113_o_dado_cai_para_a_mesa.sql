-- ════════════════════════════════════════════════════════════════════════════
-- Mesa — 0113 · o dado cai para a mesa inteira
--
-- `dominio_atacar_como` resolve a briga assalto por assalto e monta um array
-- com CADA rolagem — os dados do atacante, os do defensor, quem perdeu quanto,
-- e como ficaram os dois territórios depois de cada troca.
--
-- Esse array era DEVOLVIDO a quem atacou, e nunca entrava no registro.
--
-- ────────────────────────────────────────────────────────────────────────────
-- NO WAR, VER O DADO CAIR É O JOGO
--
-- É a única coisa que a mesa faz junta. Todo mundo se debruça, todo mundo
-- conta em voz alta, e quem perde três exércitos em duas rodadas tem uma
-- plateia — que é metade da graça de perder.
--
-- Aqui só o atacante via. Para os outros cinco, um território mudava de cor e
-- pronto: o mapa contava o resultado sem nunca contar a história. O cliente já
-- sabia encenar a rolagem (`components/dominio/dados.tsx` existe desde o
-- começo, e o comentário no topo dele diz exatamente por quê) — o que faltava
-- era o dado chegar até os outros.
--
-- E quem se reconectasse no meio perdia a rolagem inteira, inclusive quem
-- atacou: o detalhe só existia na resposta da chamada, que não se repete.
--
-- ────────────────────────────────────────────────────────────────────────────
-- E É O CRITÉRIO DE AUDITORIA, DE QUEBRA
--
-- O PRD 04 §11 pede "cada assalto individual auditável no log", e a nota de lá
-- diz que o registro vive em `public_state.log`. Vivia tudo, menos os assaltos.
-- Agora vive.
--
-- ────────────────────────────────────────────────────────────────────────────
-- UMA LINHA POR ATAQUE, NUNCA DUAS
--
-- O registro é capado em 80 linhas, e o primeiro desenho disto acrescentava uma
-- linha `assalto` ao lado da conquista. Duas linhas por ataque cortam pela
-- metade a história que cabe ali — e o teste que confere as conquistas do
-- registro parou de achar conquista nenhuma: elas tinham saído pela janela.
--
-- Então as rolagens viajam DENTRO da linha da conquista, e a linha `assalto`
-- existe só para o ataque que não conquista — que antes não deixava rastro
-- nenhum, e é o que explica por que alguém ficou fraco no meio da partida.
--
-- `p_vezes` é no máximo 12, então a linha mais gorda possível tem doze
-- rolagens e cabe folgada.
-- ════════════════════════════════════════════════════════════════════════════
